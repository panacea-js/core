...
ava_1.default('When an entity has no label defined an error should be thrown', t => {
    ...
    t.true(testCommon_1.entityHasErrorMessage(entityTypes.getData().Cat, `A 'plural' key must be set on entity type: Cat`));
    entityTypes.clearCache();
});
ava_1.default('When an entity has no storage defined an error should be thrown', t => {
    ...
    t.true(testCommon_1.entityHasErrorMessage(entityTypes.getData().Cat, `A 'storage' key must be set on entity type: Cat`));
    entityTypes.clearCache();
});
ava_1.default('When an entity field has empty definition an error should be thrown', t => {
    ...
    t.true(testCommon_1.entityHasErrorMessage(entityTypes.getData().Cat, 'Field breakingField configuration is empty'));
});
ava_1.default('When an entity field has no type defined an error should be thrown', t => {
    ...
    t.true(testCommon_1.entityHasErrorMessage(entityTypes.getData().Cat, 'Field type not defined for breakingField'));
});
ava_1.default('When an entity field has no label defined an error should be thrown', t => {
    ...
    t.true(testCommon_1.entityHasErrorMessage(entityTypes.getData().Cat, 'Field label not defined for breakingField'));
});
ava_1.default('When field definitions key is empty an error is thrown', t => {
    hooks.once('core.entityTypes.definitions', ({ definitions }) => {
        definitions.Cat.fields = {};
    });
    t.true(testCommon_1.entityHasErrorMessage(entityTypes.getData().Cat, 'Fields do not exist on entity type: Cat'));
});
